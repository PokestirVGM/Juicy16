...
That is 83 parameters. The selected-channel `volume` and `pan` parameters that
...
None of the 64 mixer parameters belongs to a parameter group. JUCE derives a
...
and `outputLevel` spans -24 to +12 dB. `bank` reaches 255 rather than 128 because
...
The state root is `MYPLUGINSETTINGS`, the Beta 1 schema is version `5`, and the
writer persists all 83 parameter values, 16 channel records (each carrying
